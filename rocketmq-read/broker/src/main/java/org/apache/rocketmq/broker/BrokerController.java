...
public class BrokerController {
    ...
    /**
     * BrokerState状态的管理器，Storage模块的，先不看
     */
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        if (result) {
            ...
            /*
             *   持久化消费端偏移量文件 文件偏移时间戳 默认5秒,1000 * 5;
             */
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
            ...
            if (this.brokerConfig.getNamesrvAddr() != null) {
                //brokerOuterAPI 更新NameServer的AddressList
                this.brokerOuterAPI.updateNameServerAddressList(this.brokerConfig.getNamesrvAddr());
                log.info("Set user specified name server address: {}", this.brokerConfig.getNamesrvAddr());
                //是否从地址服务器寻找Name Server地址，正式发布后，默认值为false,2分钟调用一次
            } else if (this.brokerConfig.isFetchNamesrvAddrByAddressServer()) {
                //没有配置nameServerAddr，就从http端点获取NameServerAddr
                this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            ...
            if (BrokerRole.SLAVE == this.messageStoreConfig.getBrokerRole()) {
                ...
                //如果是Master节点，就打印master和slave的不同
                this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
    ...
    public String getbackUpBrokerAddr() {
        return this.brokerConfig.getBackUpBrokerIP1() + ":" + this.nettyServerConfig.getListenPort();
    }
